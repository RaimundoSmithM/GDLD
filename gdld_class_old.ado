...
if `new'==0 {
	...
	**Import metadata
	*import excel "${metadatapath}${metadatafile}", firstrow sheet("metadata") clear // Open worksheet
	import excel "https://github.com/RaimundoSmithM/GDLD/blob/master/Metadata%2026.02.xls?raw=true", firstrow sheet("metadata") clear // Open worksheet

// RS: We should not be getting errors because of this with an improved version of the metadata	
	foreach var of varlist *{
		cap confirm string variable `var', exact
		*if _rc == 0 qui replace `var' = stritrim(strtrim(ustrtrim(`var')))
	}
...
**Local survey file
cd "${surveypath}"
local surveydata: dir . files "MSI_`ccode'_`year'_`survey'*.dta" 
use `surveydata', clear	

cd "${outdir}"

global i2d2_path "C:/Users/Cristian/Documents/Trabajo/Banco Mundial 2.0/2019/hola"
local surveydata: dir "${i2d2_path}" files "MSI_`ccode'_`year'_`survey'*.dta" 
local f = "${i2d2_path}" + `surveydata'
use "`f'", clear

tempfile surveydata
filelist , dir(${i2d2_path}) pattern(MSI_`ccode'_`year'_`survey'*.dta) save(`surveydata') replace
