...
*Survey ID's
global ccode = "DJI"                                                                      // Country 3 Digit Name to replace "PAK", keep quotation marks.
global year = 2015                                                                       // 4 digit Year to replace "2014", keep keep quotation marks.
global survey = "EDESIC"        															// Survey Acro Name to replace "LFS", keep quotation marks.

global isic_dig = 4
global occup_dig = 4                                                            			// Industry digit of aggregation
...
/*
	...
	Input Files: 	- isic_corr(integer)	: Integer from 1 to 6 that corresponds to the concordance tables that must be used according to 
										  ...
										  7 --------------> NACE 2 - ISIC Rev. 4 concordance table
...
gdld_classifications, isic_corr(1) isic_dig(${isic_dig}) occup_corr(3) occup_dig(${occup_dig})
	
// Add Local Classification

insheet using "${boxpath}ISIC REV. 4 - NACE REV. 2_20190617_080926.csv", clear
ren v1 isic_rev4_4dig
ren v2 industry_orig
destring isic_rev4_4dig, replace force
replace ${industry_var} = subinstr(${industry_var},".","",.)
gen d = length(${industry_var})
keep if d == 4
destring ${industry_var}, replace force
keep if !mi(isic_rev4_4dig) & !mi(${industry_var})
bys ${industry_var} isic_rev4_4dig: keep if _n == 1
keep ${industry_var} isic_rev4_4dig
save "${outpath}isic4_concordance_cnae.dta", replace
...
** Keep all I2D2 vars    
use /*if industry_orig == 113 using*/  `surveydata', clear	
...
// MERGE SURVEY DATA WITH ISIC REV. 4 CONCORDANCE TABLE 
	/*
	joinby ${industry_var} using `industry_concordance', unmatched(both)
	save "${maindir}Applications/Manufacturing skill intensity/Harmonizations/Output/isic4_concordance.dta", replace
		*/
